...
pub fn read<P: AsRef<Path>, C: Color>(path: P) -> Result<ImageBuf<u8, C>, Error> {
    ...
    match (C::name(), info.color_type) {
        ...
        ("gray", png::ColorType::RGBA) => {
            let mut image: ImageBuf<u8, Rgba> =
                ImageBuf::new(info.width as usize, info.height as usize);
            reader.next_frame(image.data_mut())?;
            let mut dest: ImageBuf<u8, C> =
                ImageBuf::new(info.width as usize, info.height as usize);
        ...
        ("rgb", png::ColorType::Grayscale) => {
            let mut image: ImageBuf<u8, Gray> =
                ImageBuf::new(info.width as usize, info.height as usize);
            reader.next_frame(image.data_mut())?;
            let mut dest: ImageBuf<u8, C> =
                ImageBuf::new(info.width as usize, info.height as usize);
...
pub fn write<P: AsRef<Path>, C: Color, I: Image<u8, C>>(
    filename: P,
    image: &I,
) -> Result<(), Error> {
