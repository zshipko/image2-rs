...
use color::{Color, Rgb, Gray, RgbToRgba};
use error::Error;
use image_buf::ImageBuf;
use image::{Image, Layout};
use filter::{Filter, ToGrayscale, ToColor};
...
pub fn read<P: AsRef<Path>, C: Color>(path: P) -> Result<ImageBuf<u8, C>, Error> {
    ...
    match (C::name(), info.pixel_format) {
        ...
        ("cmyk", jpeg::PixelFormat::CMYK32) => {
            Ok(ImageBuf::new_from(info.width as usize, info.height as usize, Layout::Interleaved, pixels))
        },
        ("gray", jpeg::PixelFormat::RGB24) => {
           let tmp: ImageBuf<u8, Rgb> = ImageBuf::new_from(info.width as usize, info.height as usize, Layout::Interleaved, pixels);
        ...
        ("rgb", jpeg::PixelFormat::L8) => {
           let tmp: ImageBuf<u8, Gray> = ImageBuf::new_from(info.width as usize, info.height as usize, Layout::Interleaved, pixels);
           let mut dest: ImageBuf<u8, C> = ImageBuf::new(tmp.width(), tmp.height());
